  ...
  return (
    <>
      <div className="fixed top-0 left-0 right-0 z-[100]">
        <nav
          className={`transition-all duration-300 font-brand ${scrolled ? "bg-white/80 backdrop-blur-md py-2 shadow-[0_10px_30px_rgba(0,0,0,0.05)]" : "bg-transparent py-6"
            }`}
        >
                  ...
                  <div className="grid grid-cols-2 gap-2 w-full px-4">
                    {link.subMenu.map((sub) => (
                      <button
                        key={sub.name}
                        onClick={() => handleNavClick(sub.path)}
                        className="text-[10px] font-montserrat font-black text-brand-brown/60 hover:text-brand-primary transition-all uppercase tracking-widest border border-brand-brown/10 px-4 py-4 rounded-2xl bg-white flex items-center justify-center text-center leading-tight active:scale-95 transition-transform h-full min-h-[60px]"
                      >
                        {sub.name}
                      </button>
                    ))}
